...
std::vector<Vertex> Sphere::toMesh() const {
    std::vector<Vertex> v;

    for(size_t i = 0; i < vertices.size() - 2; i += 3)
        v.emplace_back(glm::vec3(vertices[i], vertices[i+1], vertices[i+2]), glm::vec3(normals[i], normals[i+1], normals[i+2]), glm::vec2() );
